...
PROJECT_DIR="/project"
RELEASE_KEY_FILENAME="my-release-key.jks"
KEYSTORE_FILE_NAME="keystore.properties"
GIT_SECRET_FILE_NAME="secrets.properties"
...
-v "$SECRET_DIR/$RELEASE_KEY_FILENAME":"$PROJECT_DIR/$RELEASE_KEY_FILENAME":ro \
-v "$SECRET_DIR/$KEYSTORE_FILE_NAME":"$PROJECT_DIR/$KEYSTORE_FILE_NAME":ro \
-v "$SECRET_DIR/$GIT_SECRET_FILE_NAME":"$PROJECT_DIR/$GIT_SECRET_FILE_NAME":ro \
-v gradle:"/root/.gradle" -v `pwd`:$PROJECT_DIR jangrewe/gitlab-ci-android  bash

docker exec -it $CONTAINER_NAME bash -c 'export GRADLE_USER_HOME=/root/.gradle; cd /project; chmod +x ./gradlew; echo "KEYSTOREPROPS" ; cat keystore.properties; ./gradlew clean; ./gradlew check; ./gradlew build' 
